...
const obtenerStyleMode = async () =>{
  try {
    ...
    a.then(data => {
      head.children[3].href = `http://localhost:3000/tareas/tareas-${data.mode}.css`;
      ...
      modeBtn.addEventListener('click',function (e) {
        ...
        if (div.classList.contains('light') === true) {
          ...
          head.children[3].href = `http://localhost:3000/tareas/tareas-dark.css`;
          div.classList.add('dark');
          actualizarStyleMode('dark'); //<------------------------light or dark | aplicar toggle fn
          modeBtn.innerText = 'light mode';
        } else{
          div.classList.remove('dark');
          head.children[3].href = `http://localhost:3000/tareas/tareas-light.css`;
